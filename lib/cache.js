var debug        = require('debug')('hn-item-cache')
...
HNItemCache.prototype.get = function (url, callback) {
  ...
  if (item) {
    debug('Found item in cache: %s', url);
    process.nextTick(function () { callback(null, item); });
  } else {
    debug('Missed cache item: %s', url);
    this.lookup(url, callback);
  }
};
...
HNItemCache.prototype.lookup = function (url, callback) {
  ...
  if (!req) {
    debug('Making new request for %s', url);
...
HNItemCache.prototype.update = function (url, item) {
  debug('Setting cache result for %s', url);
